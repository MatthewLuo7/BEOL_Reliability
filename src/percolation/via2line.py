...
class Via2LineSim(PlanarCapSim):
	def __init__(self,
				 ...
				 # via_dim_x:float=10.5,	# via size (x dim), equal to line size (x dim)
				 via_dim_y:float=10.5,		# via size (y dim)
				 via_dim_z:float=10.5*2,	# via size (z dim)
				 line_dim_x:float=10.5,		# line size (x dim)
		...
		assert vm_offset > -line_dim_x
		assert vm_offset <= min(line_dim_x, ll_space)
		assert ll_space > 0.
		assert via_dim_y > 0.
		assert via_dim_z > 0.
		assert line_dim_x > 0.
		...
		self.line_b_dims = [self.line_dim_x + self.ll_space,
							self.line_dim_x + self.ll_space + self.line_dim_x,
							0.,
							0. + self.line_dim_y,
							0.,
							0. + self.line_dim_z]
		...
		# ----- dims of the via -----
		self.via_dims = [max(self.vm_offset, 0.),
						 max(self.vm_offset + self.line_dim_x, 0.),
	...
	def get_sample(self, max_try:int=1000) -> np.ndarray:
		...
		for _ in range(max_try):
			point = self.rng.uniform(low=[0., 0., 0.],
					high=[self.line_dim_x*2+self.ll_space, self.line_dim_y, self.line_dim_z+self.via_dim_z])
