...
if __name__ == "__main__":
	...
	parser.add_argument('-m', '--m-param', type=float, default=5.0, help='Parameter m (> 1.0)')
	...
	fig1.savefig(data_root / dir_name / "defect_density.png")
	plt.close(fig1)
	...
	fig2.savefig(folder_path / dir_name / "defect_density_weibits.png")
	plt.close(fig2)

	fig3, ax = plt.subplots(figsize=(8, 5))
	ax.hist(sucs_breakdown_time, bins=200, density=True)
	ax.grid()
	ax.set_ylabel('Probability Density')
	ax.set_xlabel('Breakdown Time')
	ax.set_title(f'Breakdown Time Distribution; m={m_param:.2f}; rN={radius_N:.2f}')
	fig3.savefig(data_root / dir_name / f"breakdown_time_m{m_param:.2f}_rN{radius_N:.2f}.png")
	plt.close(fig3)

	fig4, ax = plt.subplots(figsize=(8, 5))
	weibull_plot(ax, sucs_breakdown_time)
	ax.set_xlabel('Breakdown Time')
	ax.set_title(f'Breakdown Time Weibits')
	fig4.savefig(folder_path / dir_name / f"defect_density_weibits_m{m_param:.2f}_rN{radius_N:.2f}.png")
	plt.close(fig4)